...
local function toggleSpectate(target)
    ...
    -- posisi awal kamera (menghadap ke part)
    Camera.CFrame = CFrame.new(
        target.Position + Vector3.new(0, 5, 10), -- jarak 10 stud di depan dan 5 stud di atas
        target.Position
    )

    -- update kamera supaya ngikutin part tapi tetap bisa diputar manual
    ...
    _G.SpectatePartConn = game:GetService("RunService").RenderStepped:Connect(function()
        ...
        -- kamera akan selalu menghadap part dari posisi relatif yang sama
        Camera.CFrame = CFrame.new(
