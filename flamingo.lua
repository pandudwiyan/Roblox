...
-- SIMPLE BLOCK SCRIPT (JUMP INPUT + LANDED) --
local UserInputService = game:GetService("UserInputService")
local ContextActionService = game:GetService("ContextActionService")

local blockOn = false
local blockConnections = {} -- simpan semua connection biar gampang putuskan
...
local function createBlock(char)
	...
	block.CFrame = CFrame.new(
		hrp.Position.X,
		hrp.Position.Y - (hrp.Size.Y/2) - 2.25,
		hrp.Position.Z
	)
...
BlockBtn.MouseButton1Click:Connect(function()
	...
	if blockOn then
		...
		-- blok saat tombol lompat ditekan (Space PC & Jump button Mobile)
		table.insert(blockConnections, UserInputService.JumpRequest:Connect(function()
			if blockOn then
				createBlock(char)
			end
		end))
		...
		-- blok saat landed
		table.insert(blockConnections, hum.StateChanged:Connect(function(_, newState)
...
-- SIMPLE BLOCK SCRIPT (JUMP INPUT + LANDED) END --
