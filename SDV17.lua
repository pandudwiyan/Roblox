...
-- ==== Toggle Spectate dengan kamera bebas (PC & Mobile) ====
local function toggleSpectate(target)
    ...
    elseif typeof(target) == "Instance" and target:IsA("BasePart") then
        ...
        local camOffset = Vector3.new(0, 5, 10) -- jarak default dari target
        local rotation = Vector2.new(0, 0)      -- rotasi X/Y
        local sensitivity = 0.003               -- kecepatan putar

        -- posisi awal kamera
        Camera.CFrame = CFrame.new(target.Position + camOffset, target.Position)

        -- koneksi input mouse & touch
        if _G.SpectateInputConn then _G.SpectateInputConn:Disconnect() end
        _G.SpectateInputConn = UIS.InputChanged:Connect(function(input)
            if spectatingTarget ~= target then return end
            if input.UserInputType == Enum.UserInputType.MouseMovement 
            or input.UserInputType == Enum.UserInputType.Touch then
                rotation = rotation + Vector2.new(-input.Delta.y, -input.Delta.x) * sensitivity
            end
        end)

        -- koneksi update kamera setiap frame
        if _G.SpectatePartConn then _G.SpectatePartConn:Disconnect() end
        _G.SpectatePartConn = game:GetService("RunService").RenderStepped:Connect(function()
            if spectatingTarget ~= target or not target.Parent then
                if _G.SpectatePartConn then _G.SpectatePartConn:Disconnect() end
                if _G.SpectateInputConn then _G.SpectateInputConn:Disconnect() end
                return
            end
            ...
            -- rotasi kamera bebas
            local rotCFrame = CFrame.Angles(0, rotation.Y, 0) * CFrame.Angles(rotation.X, 0, 0)
            local cameraPos = target.Position + (rotCFrame:VectorToWorldSpace(camOffset))
...
-- ==== Scan BaseParts yang punya script & skip nama "Part" ====
local function scanBasePartsByName()
    local buckets = {} -- name -> {parts}
    for _, inst in ipairs(Workspace:GetDescendants()) do
        if inst:IsA("BasePart") then
            -- Skip kalau namanya persis "Part"
            if inst.Name == "Part" then
                continue
            end

            -- cek kalau punya descendant script
            ...
            -- kalau nggak ada script di part ini, coba cek parent modelnya
            if not hasScript and inst.Parent then
            ...
            -- kalau lolos filter baru dimasukin
            if hasScript then
