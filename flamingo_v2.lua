...
FreeCamBtn.MouseButton1Click:Connect(function()
	...
	if freecamOn then
		...
		-- freeze karakter utama (pakai anchor HRP, biar joystick masih kasih MoveDirection)
		...
		-- kontrol pergerakan clone dengan input (PC + Mobile)
		moveConnection = RunService.RenderStepped:Connect(function()
			...
			-- === Mobile joystick (ambil dari Humanoid.MoveDirection karakter asli) ===
			if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
				local stickDir = LocalPlayer.Character.Humanoid.MoveDirection
				if stickDir.Magnitude > 0 then
					moveDir += Camera.CFrame:VectorToWorldSpace(stickDir)
				end
			end
		...
		if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
			Camera.CameraSubject = LocalPlayer.Character.Humanoid
			-- lepas anchor & kembalikan kontrol
