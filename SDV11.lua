--// Workspace & Player Scanner GUI FINAL v3
--// By ChatGPT for Delta Executor Roblox
...
Instance.new("UIScale", MainFrame).Scale = 1 -- Mobile scaling
...
local function teleportTo(object, isPlayer)
	local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
	if hrp then
		if isPlayer and object.Character and object.Character:FindFirstChild("HumanoidRootPart") then
			local targetHRP = object.Character.HumanoidRootPart
			local backOffset = -targetHRP.CFrame.LookVector * 12
			hrp.CFrame = CFrame.new(targetHRP.Position + backOffset)
...
local function toggleSpectate(object, isPlayer)
	if spectating == object then
		...
		if isPlayer and object.Character and object.Character:FindFirstChild("Humanoid") then
			camera.CameraSubject = object.Character.Humanoid
		elseif object:IsA("BasePart") then
...
local function buildList()
	...
	-- Add players category first
	table.insert(sortedCategories, 1, {Name = "Players", Count = #scanPlayers()})
		...
		catButton.MouseButton1Click:Connect(function()
			if expanded then
				...
				if cname == "Players" then
					for i, plr in ipairs(scanPlayers()) do
						...
						itemLabel.Text = plr.Name .. " | " .. plr.DisplayName
