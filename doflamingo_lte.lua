-- Floating Tools UI (PC + Mobile) - Ghost Sphere Rev 11 (Bug Fixes)
-- Fitur: Vision, Jump, Ghost, Teleport/Marking, Player List (Sort/Search), UI Rapi
-- Fix: Player Count Title, Search Scroll Reset, Infinite Scroll Fix
...
panel.Size = UDim2.new(0, 245, 0, 95) -- Disesuaikan agar pas dengan 3 tombol
...
-------------------------------------------------
-- BUTTON CREATOR (Presisi)
-------------------------------------------------

local BTN_WIDTH = 75
local BTN_HEIGHT = 28
local BTN_GAP = 5 -- Jarak antar tombol
local START_X = 5
...
-- Baris 2 (Y = 62) - Warna disesuaikan
...
-------------------------------------------------
-- PLAYER LIST GUI (Rapih & Sejajar)
-------------------------------------------------

local playerListPanel = Instance.new("Frame")
playerListPanel.Name = "PlayerListPanel"
playerListPanel.Size = UDim2.new(0, 480, 0, 300) -- Sedikit diperkecil agar compact
...
plTitle.Text = "Player (0 Player)" -- Default text
...
-- Definisi Lebar Kolom (Sama untuk Header dan Baris)
local colWidths = {35, 115, 115, 60, 65, 65} -- No, Name, Nick, Dist, TP, Cam
...
-------------------------------------------------
-- PLAYER LIST SYSTEM (Bug Fixes Applied)
-------------------------------------------------
...
local function createPlayerRow(plr, index)
    ...
    -- Generate Labels & Buttons based on colWidths
    -- 1. No
    ...
    tpB.Position = UDim2.new(0, xPos + 5, 0, 2) -- Centered with +5 offset
...
-- Search Logic (Fixed: Reset Scroll & Update Canvas)
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    -- Fix Bug Scroll Search: Reset posisi scroll ke atas agar hasil terlihat
...
local function initPlayerList()
    ...
    -- Fix Bug Title: Update jumlah player
    plTitle.Text = "Player ("..(i-1).." Player)"
    
    -- Fix Bug Scroll: Pastikan CanvasSize mengikuti konten
    scrollingFrame.CanvasSize = UDim2.new(0, 0, 0, listLayout.AbsoluteContentSize.Y)
end

-- Listener for Canvas Size (Fix Scroll Mentok)
