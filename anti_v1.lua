...
local function teleportBehind(targetPlayer)
    ...
    if targetHRP and myHRP then
        local behind = targetHRP.CFrame * CFrame.new(0, 0, 12)
...
-- Toggle AntiGrav (revisi loncat bertingkat)
local function toggleAntiGrav()
    ...
    if antiGravActive then
        ...
        if hum and hrp then
            antiGravHeight = hrp.Position.Y

            -- Listener ketika loncat → update tinggi awal
            ...
            -- Cegah turun lebih rendah dari titik loncat
            antiGravConnection = RunService.Heartbeat:Connect(function()
                if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
                    local hrp = LP.Character.HumanoidRootPart
                    if hrp.Position.Y < antiGravHeight then
                        hrp.CFrame = CFrame.new(hrp.Position.X, antiGravHeight, hrp.Position.Z)
                    end
                end
            end)
        end
